...
class ECAPA_gender(nn.Module, PyTorchModelHubMixin):
    ...
    def logtorchfbank(self, x: torch.Tensor, device: torch.device) -> torch.Tensor:
        # Preemphasis
        flipped_filter = torch.FloatTensor([-0.97, 1.]).unsqueeze(0).unsqueeze(0).to(device)
        x = x.unsqueeze(1)
        ...
        mel_spectrogram = mel_spectrogram.to(device)  # MelSpectrogram 객체가 GPU에서 동작하도록 수정
    ...
    def forward(self, x : torch.Tensor) -> torch.Tensor:
        x = self.logtorchfbank(x,x.device)
    ...
    def predict(self, audio : torch.Tensor, device: torch.device) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            logit, pred = output.max(1)
        return self.pred2gender[pred.item()],logit.item()
    
class ECAPA_gender_age(nn.Module, PyTorchModelHubMixin):
    def __init__(self, C : int = 1024):
        super(ECAPA_gender_age, self).__init__()
        self.C = C
        self.conv1  = nn.Conv1d(80, C, kernel_size=5, stride=1, padding=2)
        self.relu   = nn.ReLU()
        self.bn1    = nn.BatchNorm1d(C)
        ...
        #self.age_linear = nn.Linear(192, 1)
        self.pred2gender = {0 : 'male', 1 : 'female'}
    ...
    def forward(self, x : torch.Tensor) -> torch.Tensor:
        ...
        age_output = output[:, 0]   # 첫 번째 출력은 나이 예측에 사용
        gender_output = output[:, 1]  # 두 번째 출력은 성별 예측에 사용
